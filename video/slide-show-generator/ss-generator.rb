...
OptionParser.new do |opts|
  opts.banner = "Usage: #{$PROGRAM_NAME} [options] <input_folder> output.mp4"
  ...
  opts.on("--extension", "images extensions (default: #{options.extension})") do |e|
  options.extension = e
  end
...
input_folder = ARGV[-2]
puts(input_folder)
input_folder += "/**/" + "*." + options.extension
input_files = Dir.glob(input_folder).select { |e| File.file? e }
input_files = input_files.sort!
output_file = ARGV[-1]
